...
TEST(vcTiff, LoadTiffFromMemory)
{
  ...
  int64_t fileSize = 0;
  vcMemoryStream *pStream = nullptr;
  TIFF *pTif = nullptr;
  uint32 w = 0;
  uint32 h = 0;
  size_t npixels = 0;
  ...
  do
  {
    ...
    result = vcTiffHandler_CreateMemoryInStream(pData, size_t(fileSize), &pStream);
    EXPECT_EQ(result, udR_Success);
    if (result != udR_Success)
      break;

    result= vcTiffHandler_CreateTiffInStream(pStream, &pTif);
    EXPECT_EQ(result, udR_Success);
    if (result != udR_Success)
      break;
